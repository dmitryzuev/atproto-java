 ...
 *
 * DAG-CBOR rules followed:
 * - Map keys are sorted in canonical CBOR order (length then lexicographic).
 * - CID links are encoded as CBOR tag 42 wrapping bytes [0x00 || cid.toBytes()].
...
public final class CborCodec {
    ...
    private CBORObject encodeMap(LexMap map) {
        // Use ordered map — keys sorted in DAG-CBOR canonical order (by CBOR-encoded key length, then lexicographic)
