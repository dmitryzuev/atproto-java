...
/**
 * Validates {@link LexValue} instances against Lexicon schema definitions.
 * Uses lazy-clone: modifies maps only when defaults are applied.
 */
public final class LexiconValidator {
    ...
    public ValidationResult validateXrpcParams(String nsid, LexXrpcParameters params, LexValue value) {
    ...
    ValidationResult validateDef(String lexUri, LexUserType def, LexValue value) {
        return switch (def) {
            ...
            case io.atproto.lexicon.schema.LexArray arr -> validateArray(lexUri, arr, value);
            case LexBlob blob          -> validateBlob(lexUri, blob, value);
            case io.atproto.lexicon.schema.LexBoolean b -> validateBooleanDef(lexUri, b, value);
            case io.atproto.lexicon.schema.LexInteger i -> validateIntegerDef(lexUri, i, value);
            case io.atproto.lexicon.schema.LexString s  -> validateStringDef(lexUri, s, value);
            case io.atproto.lexicon.schema.LexBytes by  -> validateBytesDef(lexUri, by, value);
    ...
    private ValidationResult validateObject(String lexUri, LexObject def, LexMap map, boolean allowExtra) {
    ...
    private ValidationResult validateArray(String lexUri, io.atproto.lexicon.schema.LexArray def, LexValue value) {
    ...
    private ValidationResult validateBlob(String lexUri, LexBlob def, LexValue value) {
        ...
        if (!def.accept().isEmpty()) {
            ...
            if (!accepted) {
                return ValidationResult.failure(lexUri, "Blob MIME type '" + mime + "' not in accepted list");
            }
        }
    ...
    private ValidationResult validateBooleanDef(String lexUri, io.atproto.lexicon.schema.LexBoolean def, LexValue value) {
        if (!(value instanceof io.atproto.lex.data.LexBoolean b)) {
    ...
    private ValidationResult validateIntegerDef(String lexUri, io.atproto.lexicon.schema.LexInteger def, LexValue value) {
    ...
    private ValidationResult validateStringDef(String lexUri, io.atproto.lexicon.schema.LexString def, LexValue value) {
        ...
        if (def.minGraphemes() != null) {
            int g = io.atproto.lex.data.util.Utf8Utils.graphemeLength(str);
            if (g < def.minGraphemes()) {
                return ValidationResult.failure(lexUri, "String too short (graphemes): min " + def.minGraphemes());
            }
        }
        if (def.maxGraphemes() != null) {
            int g = io.atproto.lex.data.util.Utf8Utils.graphemeLength(str);
            if (g > def.maxGraphemes()) {
                return ValidationResult.failure(lexUri, "String too long (graphemes): max " + def.maxGraphemes());
            }
        }
        ...
        if (def.format() != null && !StringFormatValidator.isValid(def.format(), str)) {
            return ValidationResult.failure(lexUri, "String does not match format '" + def.format() + "': " + str);
        }
        return ValidationResult.success(value);
    }

    private ValidationResult validateBytesDef(String lexUri, io.atproto.lexicon.schema.LexBytes def, LexValue value) {
    ...
    private ValidationResult validateUnion(String lexUri, LexRefUnion union, LexValue value) {
        ...
        if (!union.refs().isEmpty()) {
            boolean matched = union.refs().stream()
                    .anyMatch(r -> lexicons.resolveLexUri(r, lexUri).equals(typeName)
                            || r.equals(typeName));
    ...
    private LexValue applyDefault(LexUserType def) {
        return switch (def) {
            case io.atproto.lexicon.schema.LexBoolean b when b.defaultValue() != null ->
                    LexBoolean.of(b.defaultValue());
            case io.atproto.lexicon.schema.LexInteger i when i.defaultValue() != null ->
                    new LexInteger(i.defaultValue());
            case io.atproto.lexicon.schema.LexString s when s.defaultValue() != null ->
