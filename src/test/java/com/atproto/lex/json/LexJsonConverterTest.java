...
class LexJsonConverterTest {
  ...
  @Test
  void roundTripNull() {
    assertThat(converter.jsonToLex(converter.lexToJson(LexNull.INSTANCE)))
        .isEqualTo(LexNull.INSTANCE);
  }

  @Test
  void roundTripBoolean() {
    assertThat(converter.jsonToLex(converter.lexToJson(LexBoolean.TRUE)))
        .isEqualTo(LexBoolean.TRUE);
    assertThat(converter.jsonToLex(converter.lexToJson(LexBoolean.FALSE)))
        .isEqualTo(LexBoolean.FALSE);
  }
  ...
  void rejectsFloatingPointNumbers() {
    assertThatThrownBy(() -> converter.jsonStringToLex("3.14")).isInstanceOf(LexError.class);
  }
}
