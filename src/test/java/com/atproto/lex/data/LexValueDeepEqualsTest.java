...
class LexValueDeepEqualsTest {
  ...
  @Test
  void scalarsEqual() {
    assertThat(LexValueDeepEquals.deepEquals(new LexString("hello"), new LexString("hello")))
        .isTrue();
  ...
  void bytesEqual() {
    assertThat(
            LexValueDeepEquals.deepEquals(
                new LexBytes(new byte[] {1, 2}), new LexBytes(new byte[] {1, 2})))
        .isTrue();
    assertThat(
            LexValueDeepEquals.deepEquals(
                new LexBytes(new byte[] {1}), new LexBytes(new byte[] {2})))
        .isFalse();
  }
}
