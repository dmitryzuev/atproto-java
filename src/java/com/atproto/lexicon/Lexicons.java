...
import com.atproto.lexicon.validation.*;
import java.util.*;

/**
 * Registry of AT Protocol Lexicon documents. Provides definition lookup and runtime validation.
 *
 * <p>Not thread-safe — external synchronization required for concurrent add/remove.
 */
public final class Lexicons implements Iterable<LexiconDoc> {
  ...
  /**
   * Resolves a lexicon URI reference against a base URI. Rules: "#fragment" → baseUri (without any
   * existing fragment) + "#fragment" "nsid#name" → "lex:nsid#name" "nsid" (no #) → "lex:nsid#main"
   * "lex:..." → unchanged
   */
