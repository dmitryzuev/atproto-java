...
/**
 * Validates AT Protocol string format constraints. See:
...
public final class StringFormatValidator {
  ...
  private static final Pattern DID = Pattern.compile("^did:[a-z]+:[a-zA-Z0-9._:%-]+$");
  ...
  private static final Pattern NSID =
      Pattern.compile(
          "^[a-zA-Z]([a-zA-Z0-9-]{0,62}[a-zA-Z0-9])?(\\.[a-zA-Z]([a-zA-Z0-9-]{0,62}[a-zA-Z0-9])?)+$");

  // AT-URI: at://authority/collection/rkey
  private static final Pattern AT_URI =
      Pattern.compile(
          "^at://[a-zA-Z0-9._:%-]+(/[a-zA-Z0-9._~:@!$&'()*+,;=%-]*)?(\\?[a-zA-Z0-9._~:@!$&'()*+,;=/?%-]*)?(#[a-zA-Z0-9._~:@!$&'()*+,;=/?%-]*)?$");
  ...
  private static final Pattern TID = Pattern.compile("^[2-7a-z]{13}$");

  // Record key: printable ASCII excluding whitespace, at-signs, and slashes
  private static final Pattern RECORD_KEY = Pattern.compile("^[a-zA-Z0-9_~.:-]{1,512}$");
  ...
  /**
   * Validates a value against a named format string. Returns true if valid or if the format is
