...
/**
 * Stack-based structural deep-equality for LexValue — avoids StackOverflowError on deeply nested
 * data. Cycle detection via IdentityHashMap prevents infinite loops.
 */
public final class LexValueDeepEquals {
  ...
  public static boolean deepEquals(LexValue a, LexValue b) {
    ...
    while (!stack.isEmpty()) {
      ...
      long pairKey =
          ((long) System.identityHashCode(x) << 32) | (System.identityHashCode(y) & 0xFFFFFFFFL);
      ...
      switch (x) {
        ...
        case LexBoolean bx -> {
          if (bx.value() != ((LexBoolean) y).value()) return false;
        }
        case LexInteger ix -> {
          if (ix.value() != ((LexInteger) y).value()) return false;
        }
        case LexString sx -> {
          if (!sx.value().equals(((LexString) y).value())) return false;
        }
        case LexBytes bx -> {
          if (!Arrays.equals(bx.value(), ((LexBytes) y).value())) return false;
        }
        case LexCidLink lx -> {
          if (!lx.cid().equals(((LexCidLink) y).cid())) return false;
        }
        case LexBlobRef brx -> {
          if (!brx.ref().equals(((LexBlobRef) y).ref())) return false;
        }
        case LexArray ax -> {
